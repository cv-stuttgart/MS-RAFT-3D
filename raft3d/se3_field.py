...
class SE3BuilderInplace(torch.autograd.Function):
    @staticmethod
    def forward(ctx, se3, ae, pts, target, weight, intrinsics, radius):
    ...
    def backward(ctx, grad_H, grad_b):
        ...
        return None, ae_grad, None, target_grad, weight_grad, None, None


class SE3Builder(torch.autograd.Function):
    @staticmethod
    def forward(ctx, attn, se3, pts, target, weight, intrinsics, radius):
        """ Build linear system Hx = b """
        ctx.radius = radius
    ...
    def backward(ctx, grad_H, grad_b):
        ...
        return attn_grad, None, None, target_grad, weight_grad, None, None
...
def step(Ts, ae, target, weight, depth, intrinsics, lm=.0001, ep=10.0, radius=32):
    ...
    H, b = SE3Builder.apply(attn, se3, pts, target, weight, intrinsics, radius)
...
def step_inplace(Ts, ae, target, weight, depth, intrinsics, lm=.0001, ep=10.0, radius=32):
    ...
    # build the linear system
    H, b = SE3BuilderInplace.apply(se3, ae, pts, target, weight, intrinsics, radius)
